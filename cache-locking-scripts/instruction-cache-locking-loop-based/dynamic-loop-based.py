...
for insn in program_insns:
    ...
    if opened_window:
        ...
        if hex(translated_address) not in hotness_dict_per_windows[current_window]:
            hotness_dict_per_windows[current_window][hex(translated_address)] = 1
        else:
            hotness_dict_per_windows[current_window][hex(translated_address)] = hotness_dict_per_windows[current_window][hex(translated_address)]+1
...
for node in selected_nodes.keys():
    print("lockingDurationsTable[" + node + "]=" + str(max(locked_lines_durations[node])) + ";")
