...
for insn in program_insns:
        ...
        if loop_exits_dict[hex(insn_address)][0] != None and loop_stack[-1] != hex(insn_address):
                ...
                if loop_stack[-1] != 'start':
                    for node in temp_window_counter:
                        if node not in conflict_graph[loop_stack[-1]][2]:
                            conflict_graph[loop_stack[-1]][2][node] = temp_window_counter[node]
                        else:
                            #if conflict_graph[loop_stack[-1]][2][node] < temp_window_counter[node]:
                            conflict_graph[loop_stack[-1]][2][node] += temp_window_counter[node]
                    
                    temp_window_counter={}

                if loop_stack[-1] != 'start' and hex(insn_address) not in conflict_graph[loop_stack[-1]][1]:
                    conflict_graph[loop_stack[-1]][1].append(hex(insn_address))
    ...
    if len(loop_stack) != 0 and loop_stack[-1] != 'start':
        ...
        conflict_graph[loop_stack[-1]][0][accessed_set].add(hex(translated_address))
